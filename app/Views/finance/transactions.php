...
<div class="w-full">
  ...
  <!-- Filter Menu -->
  <form method="GET" action="" class="flex flex-col sm:flex-row gap-3 bg-white p-4 rounded-2xl border border-slate-100 mb-4 shadow-soft-sm">
    <input type="hidden" name="page" value="transactions">
    <div class="flex-[2]">
      <label class="text-3xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">Pencarian</label>
      <input type="text" name="search" value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>" placeholder="Cari keterangan, no trx..." class="w-full text-xs px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-purple-500">
    </div>
    <div class="flex-1">
      ...
      <select name="account_id" class="w-full text-xs px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-purple-500 bg-white">
        ...
        <?php foreach ($accounts as $acc): ?>
          <option value="<?php echo $acc['id']; ?>" <?php echo (isset($_GET['account_id']) && $_GET['account_id'] == $acc['id']) ? 'selected' : ''; ?>>
            <?php echo Helper::e($acc['account_name'] . ' (' . $acc['bank_name'] . ')'); ?>
          </option>
        <?php endforeach; ?>
      </select>
    </div>
  ...
  <!-- Tabs Navigation -->
  <div class="flex items-center gap-4 border-b border-slate-200 mb-6 px-4 bg-white pt-2 rounded-t-2xl shadow-soft-sm overflow-x-auto hide-scrollbar">
    ...
    <form method="POST" action="<?php echo Helper::url('transactions'); ?>" class="space-y-4">
      ...
      <div>
        ...
        <select name="account_id" id="edit_trx_account_id" required class="w-full text-xs px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-purple-500 bg-white">
          <?php foreach ($accounts as $acc): ?>
            <option value="<?php echo $acc['id']; ?>"><?php echo Helper::e($acc['account_name']); ?></option>
          <?php endforeach; ?>
        </select>
      </div>
